...
-- =============================================
-- [문제]
--  현재 정책이 `select using (true)` 라서, 브라우저에 공개된 anon 키만 있으면
...
--
-- [해결]
--  1) 로그인 검증을 서버측 함수(RPC)로 옮긴다 → PIN/비밀번호를 클라이언트로 절대 내려보내지 않음
--  2) anon 에게는 민감 컬럼을 제외한 컬럼만 SELECT 권한을 준다 (컬럼 단위 GRANT)
--
-- ⚠ 실행 순서: 이 SQL을 먼저 실행한 뒤 최신 코드를 배포하세요.
--    (RPC 가 먼저 있어야 새 코드가 동작합니다. 구 코드도 잠시 함께 동작합니다.)
...
-- 4. 확인
-- ─────────────────────────────────────────────
-- 아래는 실행 후 터미널에서 검증해보세요. 두 요청 모두 실패해야 정상입니다.
